...
const OrderManagement = () => {
  ...
  const filteredOrders = orders.filter((order) =>
    [order.id, order.customer, ...order.products.map((p) => p.name)].some(
              ...
              {filteredOrders.map((order) => {
                ...
                return (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">
                      <span
                        className={`inline-flex items-center px-2 py-1 text-xs rounded-full ${getStatusColor(
                          order.status
                        )}`}
                      >
            ...
            <div className="space-y-6 text-sm text-gray-700">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <p>{formatDate(selectedOrder.bookingDate)}</p>
                  </div>
                  <div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <p>₹{Number(selectedOrder.total || 0).toFixed(2)}</p>
                  </div>
                  <div>
                    <label className="font-medium">Advance Paid</label>
                    <p>₹{Number(selectedOrder.paidprice || 0).toFixed(2)}</p>
                  </div>
                  <div>
              ...
              <div>
                ...
                {selectedOrder.products.map((product, idx) => (
                  <div
                    key={idx}
                    className="grid grid-cols-1 gap-2 p-3 mb-3 border rounded sm:grid-cols-2 bg-gray-50"
                  >
                    <div>
                      <label className="font-medium">Name</label>
                      <p>{product.name}</p>
